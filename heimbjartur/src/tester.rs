...
pub async fn start_testing(tester: Tester) -> Result<(), anyhow::Error> {
    ...
    let test_suite_count = match tester.mode {
        TesterMode::Single { src, dst, protocol, expect } => {
            ...
            let src = util::parse_socket_address_with_fallback(src)?;
            let dst = util::parse_socket_address_with_fallback(dst)?;
            ...
            let test_packet = TestPacket::new(src, dst, protocol, expect);
            test_single(test_packet)
        }
    ...
    if failures.is_empty() {
        ...
        warn!("Result: {}/{} tests passed", test_suite_count - failures.len(), test_suite_count);
    }
